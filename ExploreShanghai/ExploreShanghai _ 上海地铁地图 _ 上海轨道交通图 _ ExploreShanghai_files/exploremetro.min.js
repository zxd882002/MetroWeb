...
$("canvas.seg").each(function(i,canvas) {
	var s=48;
	var c=24;
	var pLineWidth=16;
	var cLineWidth=12;
	...
	var ExploreMap=function(options) {
		...
		var polymap = null;
		var route_url = "/route";
		var mp3_url = "http://www.exploremetro.com/static/mp3/"+city+"/";
		...
		function initDB() {
		    if (undefined==this["db"]) {
		        ...
		        stationsDB = [];
    		    $("#stationdb li").each(function(i,item){stationsDB.push($(item).data("station"))});
    		    
    		    linesDB = [];
    		    $("#linedb li").each(function(i,item){linesDB.push($(item).data("line"))});
    		    
		    } else {
		    //pedia - read data from javascript
    		    stationsDB = db["stations"];
    		    linesDB = db["lines"];
		    } 
		...
		function coordsForStation(obj) {
			...
			var xy = {zoom:ZOOM_MED,column:(x/s+xd)/tileSize,row:(yd-y/s)/tileSize};
		...
		function displayPopoverAt(near) {
			...
			polymap.center({lon:coords[0],lat:coords[1]});
			var size = polymap.size();
			$("#popover").fadeIn();
			var offset = {left: size["x"]/2-146, top: size["y"]/2-$("#popover").height()+10};
			$("#popover").offset(offset)
			var nameStr = near[nameCode];
			if (showCodes) {
			    nameStr+=(" ("+near["code"]+")");
			}
			$("#popover").find("a#metropedia").html(nameStr);
			$("#popover").find(".alt_name").html(near["name_"+languageComplement]);
			$("#popover").find("a#metropedia").attr("href",pedia_url+"?lang="+language+"&id="+near["exploreid"]);
			$("#popover .lineholder").empty();
			var str = near["lines"];
			$("#popover .lineholder").removeClass("superchange");
			for (var i=0;i<str.length;i++) {
			    var lineCode = str.substr(i,1);
			    var line = lineByCode[lineCode];
			    var li = liForLine(line);
			    if (i>=2) {
			        $("#popover .lineholder").addClass("superchange");
			    }
			    $("#popover .lineholder").append(li);
			}
			var audio = $("#popover").find("audio");
			var mp3 = mp3_url+near["exploreid"].replace(/,/g,"-")+".mp3";
			audio.find("source").remove()
			$("<source />").attr("src",mp3).attr("type","audio/mpeg").appendTo(audio);
		}
		...
		function geoJsonLoaded(e) {
            for (var i = 0; i < e.features.length; i++) {
                ...
                $(elt).data("station",e.features[i]["data"]["properties"]);
                try {
									$(elt).addClass("dot");
								} catch (e) {
                ...
                $(elt).mouseup(function(event) {
                    if (quickDragInProgress && quickDragElement2==this) {
                        startRoute();
                        endRoute($(quickDragElement2).data("station"));
                    } else {
                        clickStation($(this).data("station"));
                    }
                });
                ...
                $(elt).mouseout(function() {
                    if (quickDragInProgress) {
                        if (quickDragElement1==null) {
                            ...
                            clearRoute(true);
                            selectedStation=$(quickDragElement1).data("station");
                            stationsLayer.reshow();
                        } else {
                ...
                $(elt).mouseover(function(event) {
                    if (quickDragInProgress && this!=quickDragElement1) {
                        ...
                        if (quickDragElement1!=quickDragElement2) {
                            ...
                            var c1 = coordsForStation($(quickDragElement1).data("station"));
                            var xy1 = polymap.locationPoint({lon:c1[0],lat:c1[1]});
                            
                            var c2 = coordsForStation($(quickDragElement2).data("station"));
                            var xy2 = polymap.locationPoint({lon:c2[0],lat:c2[1]});
		...
		function initMap() {
            ...
            var urlBuilder = function(c) {
                ...
                return TILEBASE+"tile_"+language+"_"+z2+"x"+c.column+"x"+c.row+".png?v="+tileVersion;
            };
            ...
            for (var i=0;i<stationsDB.length;i++) {
                ...
                var pt = {geometry: {coordinates: coords, type: "Point"}, properties:stationsDB[i]};
                featuresArray.push(pt);
            }
            
            var stylist = po.stylist().attr("fill", function(d) { 
                var isHighlighted = routeArr.indexOf(d.properties)>-1 || d.properties==selectedStation;
            ...
            polymap.center(polymap.coordinateLocation({zoom:ZOOM_MED,column:W/2/tileSize,row:H/2/tileSize}));
		}
		...
		function updateFlags() {
		    ...
		    var flags = [];
		    if (startStation!=null) {
		        flags.push({geometry: {coordinates: coordsForStation(startStation), type: "Point"}});
	        } 
	        if (endStation!=null) {
		        flags.push({geometry: {coordinates: coordsForStation(endStation), type: "Point"}});
	        }
		...
		function nameTo(obj,stn,prefix,deftext) {
		    var span = $(obj+">span");
		    if (stn==null) {
		        span.html(prefix+deftext);
		    } else {
    		    var nameStr = stn[nameCode];
    		    if (showCodes) {
        		    nameStr+=(" ("+startStation["code"]+")");
        		}
    		    span.html(prefix+nameStr);
		    }
		    $(obj).textfill({ maxFontPixels: 21 });
		}
		function endRoute(station) {
		    ...
		    updateFlags(2);
			updateStationNames();
			var fromto={from:startStation["exploreid"],to:endStation["exploreid"],lang:language};
			$.post(route_url,fromto,function(data, textStatus, jqXHR){
				var time=parseInt($(data).find("time").text());
				var price=parseFloat($(data).find("price").text());
				var route=$(data).find("route").text();
				var html=$(data).find("details").text();
				var priceFormatted = sprintf(currencyFormat,price/currencyDivisor);
				showJourney(time,priceFormatted,route,price>0,html);
			},"xml");
			
		}
		function showJourney(time,price,route,showPrice,html) {
			currentJourneyUI.find(".time").html(sprintf(i18n.MINS,time));
			if(showPrice) {
			    currentJourneyUI.find(".price").html(price);
		    } else {
		        currentJourneyUI.find(".price").html("");
		    }
			var ids = route.split(",");
			routeArr = [];
			...
			stationsLayer.reshow();
			$(".journeyresult").show();
			$("#routedetailsajax").empty();
		    $("#routedetailsajax").html(html);
			
		}
	
		function showTimes(stn) {
		    $("#times").slideLeftShow();
		    $("#times").find(".timesstation").html(stn[nameCode]);
		    $("#timesajax").empty();
		    $("#timesajax").load("times/?exploreid="+stn["exploreid"]);
		}
		function initPopover() {
			$("#plan_route_btn").click(function() {
				$("#popover").fadeOut();
				startRoute();
				return false;
			});
		...
		function clearRoute(hideJourneyView) {
			...
			$("#searchresults").empty();
			$("#routedetailsajax").empty();
			$("body").removeClass("searchInProgress");
    		clearFlags();
		...
		function showSearch(search) {
		    ...
		    for (var i = 0;i<results.length;i++) {
		        var li = $("<li>"+bolden(results[i][nameCode],search)+"<ul class=\"lineholder\"></ul></li>");
		        var str = results[i]["lines"];
    			for (var j=0;j<str.length;j++) {
    			    var lineCode = str.substr(j,1);
    			    var line = lineByCode[lineCode];
    			    li.find(".lineholder").append(liForLine(line));
			    }
		        
		        li.addClass("sr");
		        li.addClass("item");
		        li.data("station",results[i]);
		        li.click(function() {
		            $("form#search input").val("");
		            goSearchStation($(this).data("station"));   
		        });
		        li.mouseenter(function() {
		            selectedSearchIndex = $("#searchresults li.sr").index($(this));
		...
		function moveSearchHighlight(diff) {
		    ...
		    var lis = $("#searchresults li.sr");
		    if (selectedSearchIndex>=lis.length) {
		        selectedSearchIndex=lis.length-1;
		    }
		    $("#searchresults li.selectedSearch").removeClass("selectedSearch");
		    $(lis[selectedSearchIndex]).addClass("selectedSearch");
        }
		function initSearch() {
            ...
            $(".searchbutton button").click(function() {
                $("body").addClass("searchInProgress");
                $("form#search input").focus();
            });
            
		}
		function goSelectedSearch() {
		    if ($("li.selectedSearch")) {
		        goSearchStation($("li.selectedSearch").data("station"));
	        }
		}
		...
		function initMenu() {
		    var submenus = $("#menu li.submenu>a");
		...
		function initLegend() {
		    for (var i=0;i<linesDB.length;i++) {
		        $("#legend .lineholder").append(liForLine(linesDB[i]));
		    }
		}
		function liForLine(line) {
	        ...
	        a.attr("title",name);
	        li.append(a);
	        li.css("color","#"+line["textColor"]);
	        li.css("background-color","#"+line["color"]);
	        return li;
		}
		function initZoom() {
		...
		function initPreload() {
		    $([staticPath+'/png/btn-route.png',staticPath+'/png/btn-traintimes.png',staticPath+'/png/flag@2x.png']).preload();
		}
		function initScroll() {
		...
		function hideAddressBar() {
              var extra = 60;
              $("body").css("height",(window.outerHeight+extra)+"px");
              setTimeout( function(){ window.scrollTo(0, 1); }, 100 );
              moveFooter();

        }
        function moveFooter() {
            $('footer').css("top",(window.pageYOffset + window.innerHeight - $('footer').height()) + 'px');
        }
        function flipAndroid() {
            var alink = $("#androidlink");
            var img = $(".iphonemenu img");
            var link = img.attr("src");
            link = link.replace("appstore","googleplay");
            img.attr("src",link);
            var oldtext = img.attr("alt");
            img.attr("alt",alink.html())
            var oldlink = img.parent().attr("href");
            img.parent().attr("href",alink.attr("href"));
            alink.attr("href",oldlink);
            alink.html(oldtext);
        }
		function initHash() {
